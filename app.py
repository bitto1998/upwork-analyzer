...
st.title("🚀 Upwork Competitor Analyzer")
...
if st.button("Start Scraping and Analyzing"):
    with st.spinner("AI is thinking... Give it 1 to 3 minutes. Do not close the page."):
        ...
        t1 = Task(
            ...
            agent=agent_buyer,
            expected_output="List of 5 search queries"
        )
        t2 = Task(
            description='Use the search tool on the queries from Task 1 to read snippets of high-ranking Upwork profiles.',
            agent=agent_scraper,
            expected_output="Rough summary of competitor profiles"
        )
        t3 = Task(
            description=(
                'Create a Master Blueprint for the user with: '
                '1) Top 10 Keywords, '
                '2) Formatting tips (paragraphs vs bullets), '
                '3) A copy-paste example intro paragraph based ONLY on the competitors.'
            ),
            agent=agent_seo,
